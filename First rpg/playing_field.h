...
public:
	playing_field() : x_dim(10), y_dim(10), game_tiles(10, std::vector<std::string>(10, ".")), starting_location({ 0,0 }), current_location({ 0,0 }) {}
	playing_field(const int _x_dim) : x_dim(_x_dim), y_dim(10), game_tiles(_x_dim, std::vector<std::string> (10, ".")), starting_location({ 0,0 }), current_location({ 0,0 }) {}
	playing_field(const int _x_dim, const int _y_dim) : x_dim(_x_dim), y_dim(_y_dim), game_tiles(_x_dim, std::vector<std::string> (_y_dim, ".")), starting_location({ 0,0 }), current_location({ 0,0 }) {}
